...
async def liste(c: Client, m: Message):
    ...
    if m.from_user.id not in admins[m.chat.id]: # if user not admin
        return await c.send_message(chat, LAN.U_NOT_ADMIN.format(ment)) # send message
    s = await c.send_message(chat, LAN.WAIT_FOR_LIST.format(ment))  # send message
    total = 0   
    deleted = 0
    bots = 0
    ...
    await c.send_document(  # send document
        ...
        caption=LAN.CAPTION_CALLTONE_TXT.format(sohbet.title, total, deleted, bots),    # caption
    )   # send document
